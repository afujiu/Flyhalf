    ...
    <v-card class="menu-card">
      <v-list class="px-0 mx-0">
        <v-list-item v-for="(item, index) in menus">
          <button @click="selectMenuKey = menus[index]" :class="{'selected':selectMenuKey == menus[index]}">{{menus[index]}}</button>
          </v-list-item>
        </v-list>
    </v-card>
    ...
    <v-card class="user-card">
      <v-container>
        <v-row>
          <v-col></v-col>
          <v-col>
            <v-btn class="x-small" color="primary" x-small flat icon="mdi-logout"></v-btn>
          </v-col>
          <v-col></v-col>
        </v-row>
      </v-container>
